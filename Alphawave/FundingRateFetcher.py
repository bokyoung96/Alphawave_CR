...
class FundingRateFetcher:
    ...
    def run(self):
        ...
        self.main_df = self.additional_data.copy()

        # NOTE: Not used while <self.deduplicate_symbols_by_volume> is not used.
...
if __name__ == "__main__":
    ...
    mkts: Exchanges to use.
    top_n: Top number of coins to search, sorted by funding fees, per exchange.
    ...
    top_n = 10
    max_workers = 10
    fetcher = FundingRateFetcher(mkts, top_n=top_n, max_workers=max_workers)
    df = fetcher.run()
    print("\nFinal Top Funding Rates:")
    print(df)

    # NOTE: Not used, for searching certain coins specifically for every exchange available.
    # coin = 'APE/USDT:USDT'
